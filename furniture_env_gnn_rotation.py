...
class FurnitureRecommendationEnvGNN(gym.Env):
    ...
    def _build_tag_based_zones(self):
        ...
        for furniture in self.existing_furniture:
            ...
            if furniture['category'] == 'seating':
                ...
                half_w = furniture['dimensions']['width'] / 2.0
                half_l = furniture['dimensions']['length'] / 2.0

                # Compute the FRONT-FACE centre of the sofa (the side that faces room)
                # Sofa is at (x,y).  "Front" = the face pointing toward room interior.
                # We determine room interior by picking the direction that moves
                # toward the room centre (room_length/2, room_width/2).
                rc_x = self.room_length / 2.0
                rc_y = self.room_width  / 2.0
                if rot in (0, 180):   # sofa long axis horizontal
                    # front is top or bottom edge
                    front_y = y + half_w if rc_y < y else y - half_w
                    front_x = x
                else:                 # sofa long axis vertical
                    front_x = x + half_l if rc_x < x else x - half_l
                    front_y = y
                # front_face = point on sofa surface facing room interior
                front_face = (front_x, front_y)

                # seating_area zone: centred on front face for companion/accessory scoring
    ...
    def _resolve_rotation(
        ...
        """
        Determine semantically correct rotation for a furniture item.

        Rules (in priority order):
        1. Wall/corner items: rotation is set later by the wall-snap / corner-snap
           blocks, so we return the agent hint unchanged here.
        2. Sofa-companion items (coffee_table, ottoman, rug): align PARALLEL to the
           nearest sofa — same rotation axis as the sofa.
        3. Seating accessories beside sofa end (magazine_rack, side_table,
           nesting_tables, floor_lamp): face TOWARD the sofa centre.
        4. Primary seating added later (armchair): face TOWARD room centre /
           toward existing sofa.
        5. Symmetric items (round/square, ratio ≈ 1): rotation doesn't matter → 0.
        6. Fallback: return agent hint.
        """
        ftype = catalog_item['type']
        dims  = catalog_item['dimensions']
        adv   = catalog_item.get('advanced_placement', {})
        p_tag = adv.get('primary_tag', '')

        # ── 1. Wall / corner items: leave rotation to the snap blocks ──────────
        wall_tags = {'wall_unit', 'wall_mounted', 'wall_hugger', 'wall_seating',
                     'leaning_decor', 'media_center'}
        sp_c = adv.get('spatial_constraints', {})
        must_wall = (sp_c.get('must_touch_wall') or sp_c.get('must_be_on_wall') or
                     sp_c.get('must_be_against_wall') or sp_c.get('must_lean_on_wall') or
                     sp_c.get('against_wall') or p_tag in wall_tags)
        corner_item = (sp_c.get('strongly_prefers_corner') or
                       sp_c.get('corner_preferred') or sp_c.get('prefers_corner'))
        if must_wall or corner_item:
            return agent_rotation   # snap blocks will override

        # ── Helper: find nearest seating (sofa / armchair) ─────────────────────
        all_furn = self.existing_furniture + self.placed_items
        seating = [f for f in all_furn if f['category'] == 'seating']

        def nearest_seating():
            if not seating:
                return None
            return min(seating,
                       key=lambda f: (x - f['position']['x'])**2 +
                                     (y - f['position']['y'])**2)

        # ── 2. Symmetric items: rotation irrelevant ─────────────────────────────
        ratio = dims['length'] / max(dims['width'], 0.01)
        # NOTE: check explicit types BEFORE ratio test — armchair ratio≈1.06
        # which falls below threshold, but armchair MUST face toward sofa.
        asymmetric_types = {'armchair', 'bench', 'console_table', 'media_cabinet',
                             'bookshelf', 'decorative_ladder', 'floor_lamp',
                             'coffee_table', 'ottoman', 'rug', 'nesting_tables',
                             'magazine_rack', 'side_table'}
        if ratio < 1.15 and ftype not in asymmetric_types:
            return 0

        # ── 3. Sofa-companion: coffee_table, ottoman, rug ──────────────────────
        #    Long axis must be PARALLEL to the nearest sofa's long axis.
        companion_types = {'coffee_table', 'ottoman', 'rug'}
        if ftype in companion_types or p_tag in ('seating_companion', 'flexible_seating',
                                                  'zone_definer'):
            ns = nearest_seating()
            if ns is not None:
                sofa_rot = ns.get('rotation', 0)
                # Sofa at rot=0/180 → long axis is horizontal (x-dir) → companion also 0
                # Sofa at rot=90/270 → long axis is vertical   (y-dir) → companion also 90
                if sofa_rot in (0, 180):
                    return 0    # long axis along x — parallel to sofa
                else:
                    return 90   # long axis along y — parallel to sofa
            return 0

        # ── 4. Seating accessories: magazine_rack, side_table, nesting_tables ──
        #    These sit at the ARM END of the sofa → perpendicular to sofa's long axis,
        #    with their face pointing TOWARD the sofa centre.
        accessory_types = {'magazine_rack', 'side_table', 'nesting_tables'}
        if ftype in accessory_types or p_tag == 'seating_accessory':
            ns = nearest_seating()
            if ns is not None:
                sx, sy = ns['position']['x'], ns['position']['y']
                sofa_rot = ns.get('rotation', 0)
                # Determine which end of the sofa we are closer to
                if sofa_rot in (0, 180):
                    # Sofa is horizontal; item is beside its left or right arm
                    # Item should run vertically (90°), face pointing inward
                    return 90 if x < sx else 270
                else:
                    # Sofa is vertical; item is above or below
                    return 0 if y < sy else 180
            return agent_rotation

        # ── 5. Floor lamp: point toward nearest seating ─────────────────────────
        if ftype == 'floor_lamp' or p_tag == 'task_lighting':
            ns = nearest_seating()
            if ns is not None:
                sx, sy = ns['position']['x'], ns['position']['y']
                dx, dy = sx - x, sy - y
                # Pick 0/90/180/270 that points most toward seating
                if abs(dx) >= abs(dy):
        ...
        # ── 6. Primary seating added later (armchair) ──────────────────────────
        if ftype == 'armchair' or p_tag == 'primary_seating':
            # Face toward existing sofa if present, else toward room centre
            sofas = [f for f in all_furn if f['type'] == 'sofa']
            if sofas:
                sx, sy = sofas[0]['position']['x'], sofas[0]['position']['y']
            else:
                sx, sy = self.room_length / 2, self.room_width / 2
            dx, dy = sx - x, sy - y
            if abs(dx) >= abs(dy):
                return 0 if dx > 0 else 180
            else:
    ...
    def step(self, action: np.ndarray) -> Tuple[np.ndarray, float, bool, bool, Dict]:
        ...
        rotation = rotation_options[rot_idx]   # agent hint, may be overridden
        ...
        # Semantic rotation: resolve correct orientation from context before snapping
        rotation = self._resolve_rotation(catalog_item, x, y, rotation)
        ...
        # Wall types: items whose SHORT dimension faces the wall (physically flat against it)
        # magazine_rack removed — it's seating_accessory (beside sofa), NOT a wall item
        wall_types = {'wall_shelf', 'bookshelf', 'console_table', 'wall_cabinet',
                      'media_cabinet', 'bench', 'decorative_ladder', 'mirror'}
        # Check all the different catalog keys used for "must be on wall"
        must_wall = (sp_c.get('must_touch_wall', False) or
                     sp_c.get('must_be_on_wall', False) or
                     sp_c.get('must_be_against_wall', False) or
                     sp_c.get('must_lean_on_wall', False) or
                     sp_c.get('against_wall', False) or
                     p_tag in ('wall_unit', 'wall_mounted', 'wall_hugger',
                               'wall_seating', 'leaning_decor', 'media_center') or
        ...
        # Corner-snap: check all three different keys used in catalog
        # plant_stand uses 'strongly_prefers_corner'
        # storage_basket uses 'corner_preferred'  (was the bug — previous fix missed this)
        # bar_cart uses 'prefers_corner'
        corner_types = {'plant_stand', 'bar_cart', 'storage_basket'}
        corner_pref = (sp_c.get('strongly_prefers_corner', False) or
                       sp_c.get('corner_preferred', False) or       # storage_basket key
                       sp_c.get('prefers_corner', False) or         # bar_cart key
                       catalog_item['type'] in corner_types)
        if corner_pref:
            corners = [
                (0.0, 0.0), (self.room_length, 0.0),
                (0.0, self.room_width), (self.room_length, self.room_width)
            ]
            nearest_corner = min(corners, key=lambda c: (x - c[0])**2 + (y - c[1])**2)
            cx, cy = nearest_corner
            half_l = catalog_item['dimensions']['length'] * scale / 2.0
            half_w = catalog_item['dimensions']['width'] * scale / 2.0
            # offset from corner so item fits inside room
            x = cx + (half_l if cx == 0.0 else -half_l)
            y = cy + (half_w if cy == 0.0 else -half_w)

        # ── Sofa-group snapping ──────────────────────────────────────────────────
        # For items that must relate to the sofa, snap them to their geometric ideal
        # position rather than leaving them anywhere in the room.
        all_furn = self.existing_furniture + self.placed_items
        sofas = [f for f in all_furn if f['type'] == 'sofa']
        if sofas:
            sofa = sofas[0]
            sx, sy  = sofa['position']['x'], sofa['position']['y']
            s_rot   = sofa.get('rotation', 0)
            s_half_l = sofa['dimensions']['length'] / 2.0
            s_half_w = sofa['dimensions']['width']  / 2.0
            # Sofa front face direction
            rc_x = self.room_length / 2.0
            rc_y = self.room_width  / 2.0
            if s_rot in (0, 180):
                front_y = sy + s_half_w if rc_y < sy else sy - s_half_w
                front_x = sx
            else:
                front_x = sx + s_half_l if rc_x < sx else sx - s_half_l
                front_y = sy

            item_half_l = catalog_item['dimensions']['length'] * scale / 2.0
            item_half_w = catalog_item['dimensions']['width']  * scale / 2.0

            if p_tag == 'seating_companion' or catalog_item['type'] in ('coffee_table', 'ottoman', 'rug'):
                # Snap directly in front of sofa front face
                direction_y = front_y - sy   # sign: +1 or -1
                direction_x = front_x - sx
                if s_rot in (0, 180):
                    snap_y = front_y + np.sign(direction_y) * (item_half_w + 0.1)
                    snap_x = sx   # centred on sofa
                else:
                    snap_x = front_x + np.sign(direction_x) * (item_half_l + 0.1)
                    snap_y = sy
                x = float(np.clip(snap_x, item_half_l, self.room_length - item_half_l))
                y = float(np.clip(snap_y, item_half_w, self.room_width  - item_half_w))

            elif p_tag == 'seating_accessory' or catalog_item['type'] in ('side_table', 'magazine_rack', 'nesting_tables'):
                # Snap beside nearest sofa arm end
                if s_rot in (0, 180):
                    # Arms at sx ± half_l; pick arm closest to current x
                    left_arm_x  = sx - s_half_l
                    right_arm_x = sx + s_half_l
                    arm_x = left_arm_x if abs(x - left_arm_x) < abs(x - right_arm_x) else right_arm_x
                    sign = -1 if arm_x < sx else 1
                    snap_x = arm_x + sign * (item_half_l + 0.1)
                    snap_y = sy
                else:
                    bottom_arm_y = sy - s_half_l
                    top_arm_y    = sy + s_half_l
                    arm_y = bottom_arm_y if abs(y - bottom_arm_y) < abs(y - top_arm_y) else top_arm_y
                    sign = -1 if arm_y < sy else 1
                    snap_x = sx
                    snap_y = arm_y + sign * (item_half_w + 0.1)
                x = float(np.clip(snap_x, item_half_l, self.room_length - item_half_l))
                y = float(np.clip(snap_y, item_half_w, self.room_width  - item_half_w))
        # ── End sofa-group snapping ──────────────────────────────────────────────

        # Dependency check: some items require another furniture type to exist first
        # e.g. table_lamp requires side_table / console_table / nesting_tables
        dep = catalog_item.get('advanced_placement', {}).get('dependency', {})
        required_types = dep.get('requires_furniture_type', [])
        if required_types:
            all_placed_types = {f['type'] for f in (self.existing_furniture + self.placed_items)}
            dep_satisfied = any(rt in all_placed_types for rt in required_types)
            if not dep_satisfied:
                # Return a step without placing — give small negative reward and continue
    ...
    def _calculate_reward(
        ...
        # Base penalty for invalid placement — scaled to match normalized reward
        if not valid_placement:
            return -3.0, components   # was -5.0 (unnormalized)
        
        # Penalty for already placed items
        if already_placed:
            components['already_placed_penalty'] = -2.0
            return -1.0, components   # was -2.0 (unnormalized)
        ...
        # 9. ROTATION CORRECTNESS
        components['rotation'] = self._reward_rotation(furniture, catalog_item)
        ...
        weights = {
            ...
            'spatial_constraints':      6.0,
            'functional_requirements':  3.5,
            'zone_preference':          3.0,
            'tag_compatibility':        2.5,
            'diversity':                2.5,
            'rotation':                 3.0,   # NEW — correct facing rewarded significantly
            'accessibility':            1.5,
        ...
        raw_reward = sum(components[k] * weights.get(k, 0) for k in components.keys())
        
        # Completion bonus — scaled down to stay within reward range
        items_so_far = len(self.placed_items)
        completion_bonus = 2.0 * (items_so_far + 1)          # was 5.0 — too large
        if items_so_far + 1 >= self.max_items:
            completion_bonus += 5.0                           # was 20.0 — caused Q-target explosion
        
        # Step efficiency penalty (minor)
        step_penalty = 0.1 * (self.current_step / self.max_steps)
        
        total_reward = raw_reward + completion_bonus - step_penalty

        # ── Normalize reward to roughly [-10, +10] so critic MSE stays finite.
        # Max possible raw_reward ≈ (5+6+3.5+3+2.5+2.5+1.5+1+1+0.8) * 1.0 + 8 + 5 ≈ 40
        # Divide by 4 so the typical good placement ≈ +8..+10
        total_reward = total_reward / 4.0

        return total_reward, components
    

    def _reward_rotation(self, furniture: Dict, catalog_item: Dict) -> float:
        """
        Reward correct orientation.

        The semantically correct rotation was already computed by _resolve_rotation()
        and baked into furniture['rotation'].  This method scores HOW WELL that
        rotation matches what a human would expect, giving the critic a clean signal.
        ...
        # Symmetric items: any rotation is fine
        if ratio < 1.15:
            return 1.0

        # Wall / corner items: rotation is set deterministically by snap blocks,
        # so we always award full score here (the snap is what matters).
        sp_c = adv.get('spatial_constraints', {})
        wall_tags = {'wall_unit', 'wall_mounted', 'wall_hugger',
                     'wall_seating', 'leaning_decor', 'media_center'}
        if (sp_c.get('must_touch_wall') or sp_c.get('must_be_on_wall') or
                sp_c.get('must_be_against_wall') or p_tag in wall_tags):
            return 1.0
        if (sp_c.get('strongly_prefers_corner') or sp_c.get('corner_preferred') or
                sp_c.get('prefers_corner')):
            return 1.0

        # ── Sofa-companion: must be PARALLEL to nearest sofa ──────────────────
        if ftype in ('coffee_table', 'ottoman', 'rug') or p_tag in (
                'seating_companion', 'flexible_seating', 'zone_definer'):
            if seating:
                ns = min(seating, key=lambda f: (x - f['position']['x'])**2 +
                                                (y - f['position']['y'])**2)
                sofa_rot = ns.get('rotation', 0)
                # Correct if both are horizontal (0/180) or both vertical (90/270)
                correct_horizontal = sofa_rot in (0, 180) and rot in (0, 180)
                correct_vertical   = sofa_rot in (90, 270) and rot in (90, 270)
                if correct_horizontal or correct_vertical:
                    return 1.0
                return 0.2   # perpendicular to sofa — penalise
            return 0.7

        # ── Seating accessories: must be PERPENDICULAR, facing sofa ───────────
        if ftype in ('magazine_rack', 'side_table', 'nesting_tables') or                 p_tag == 'seating_accessory':
            if seating:
                ns = min(seating, key=lambda f: (x - f['position']['x'])**2 +
                                                (y - f['position']['y'])**2)
                sofa_rot = ns.get('rotation', 0)
                sx, sy   = ns['position']['x'], ns['position']['y']
                # Correct rotation faces TOWARD sofa
                if sofa_rot in (0, 180):
                    expected = 90 if x < sx else 270
                else:
                    expected = 0 if y < sy else 180
                if rot == expected:
                    return 1.0
                if rot in (expected - 180) % 360:  # facing away (180° off)
                    return 0.3
                return 0.5  # perpendicular
            return 0.7

        # ── Floor lamp: face toward nearest seating ────────────────────────────
        if ftype == 'floor_lamp' or p_tag == 'task_lighting':
            if seating:
                ns  = min(seating, key=lambda f: (x - f['position']['x'])**2 +
                                                  (y - f['position']['y'])**2)
                ...
                dx, dy = sx - x, sy - y
                if abs(dx) >= abs(dy):
                    expected = 0 if dx > 0 else 180
                else:
                    expected = 90 if dy > 0 else 270
                return 1.0 if rot == expected else 0.4
            return 0.7

        # ── Armchair: face toward sofa front face (conversation area centre) ──
        if ftype == 'armchair' or p_tag == 'primary_seating':
            sofas = [f for f in all_furn if f['type'] == 'sofa']
            if sofas:
                s = sofas[0]
                sx, sy   = s['position']['x'], s['position']['y']
                s_rot    = s.get('rotation', 0)
                s_half_w = s['dimensions']['width'] / 2.0
                # Target = sofa front face (the face toward room interior)
                rc_x = self.room_length / 2.0
                rc_y = self.room_width  / 2.0
                if s_rot in (0, 180):
                    tx = sx
                    ty = sy + s_half_w if rc_y < sy else sy - s_half_w
                else:
                    ty = sy
                    tx = sx + s_half_w if rc_x < sx else sx - s_half_w
            else:
                tx, ty = self.room_length / 2, self.room_width / 2
            dx, dy = tx - x, ty - y
            if abs(dx) >= abs(dy):
                expected = 0 if dx > 0 else 180
            else:
                expected = 90 if dy > 0 else 270
            return 1.0 if rot == expected else 0.3

        return 0.8   # unknown type — mild pass
    ...
    def _reward_tag_based_placement(
        ...
        if primary_tag == 'seating_companion':
            # Must be directly in front of sofa — tight constraint
            # Ideal: 0.3–0.9m from sofa front face, centred on sofa x-axis
            for zone in self.spatial_zones['seating_area']:
                cx, cy = zone['center']       # sofa front face
                sx, sy = zone.get('sofa_center', (cx, cy))
                dist_from_front = np.sqrt((x - cx)**2 + (y - cy)**2)
                # lateral offset from sofa centreline
                if zone.get('sofa_rot', 0) in (0, 180):
                    lateral = abs(x - sx)
                    lateral_max = zone.get('sofa_half_l', 1.1) + 0.3
                else:
                    lateral = abs(y - sy)
                    lateral_max = zone.get('sofa_half_l', 1.1) + 0.3
                # ideal: 0.5m in front, within sofa width
                if dist_from_front <= 1.2 and lateral <= lateral_max:
                    score = max(score, 1.0 - (abs(dist_from_front - 0.5) / 1.2))
                elif dist_from_front <= 2.0:
                    score = max(score, 0.3)   # too far but still in front arc
                # else: 0.5 base (far away)
        
        elif primary_tag == 'primary_seating':
            # Armchair must be in front-arc of sofa:
            #   - on the SAME SIDE as the sofa front face (not behind sofa)
            #   - 1.2–3.0m from sofa front face
            #   - flanking (offset left or right), not blocking the path
            for zone in self.spatial_zones['seating_area']:
                cx, cy  = zone['center']       # sofa front face
                sx, sy  = zone.get('sofa_center', (cx, cy))
                half_l  = zone.get('sofa_half_l', 1.1)
                rot     = zone.get('sofa_rot', 0)

                dist_from_front = np.sqrt((x - cx)**2 + (y - cy)**2)

                # "Same side as front face" check:
                # front_vec points from sofa centre toward front face
                front_dx = cx - sx
                front_dy = cy - sy
                # item_vec from sofa centre to item
                item_dx  = x - sx
                item_dy  = y - sy
                # dot product > 0 means item is on the front side
                on_front_side = (front_dx * item_dx + front_dy * item_dy) > 0

                # lateral offset from sofa centreline (flanking check)
                if rot in (0, 180):
                    lateral = abs(x - sx)
                else:
                    lateral = abs(y - sy)
                flanking = lateral >= half_l * 0.5   # at least half-arm-length offset

                if on_front_side and flanking and 1.2 <= dist_from_front <= 3.0:
                    score = max(score, 0.95)
                elif on_front_side and 1.0 <= dist_from_front <= 3.0:
                    score = max(score, 0.7)   # front arc but too centred
                elif dist_from_front <= 3.5:
                    score = max(score, 0.3)   # wrong side or too far
        
        elif primary_tag == 'seating_accessory':
            # Must be at the ARM END of the sofa (beside, not in front)
            for zone in self.spatial_zones['seating_area']:
                sx, sy  = zone.get('sofa_center', zone['center'])
                half_l  = zone.get('sofa_half_l', 1.1)
                half_w  = zone.get('sofa_half_w', 0.45)
                rot     = zone.get('sofa_rot', 0)
                item_hw = furniture['dimensions']['width'] / 2.0

                if rot in (0, 180):
                    # Sofa is horizontal — arm ends are at x = sx ± half_l
                    # Ideal: beside arm, at sofa y level
                    dist_left_arm  = np.sqrt((x - (sx - half_l))**2 + (y - sy)**2)
                    dist_right_arm = np.sqrt((x - (sx + half_l))**2 + (y - sy)**2)
                    best_arm_dist  = min(dist_left_arm, dist_right_arm)
                else:
                    dist_top_arm    = np.sqrt((x - sx)**2 + (y - (sy + half_l))**2)
                    dist_bottom_arm = np.sqrt((x - sx)**2 + (y - (sy - half_l))**2)
                    best_arm_dist   = min(dist_top_arm, dist_bottom_arm)

                if best_arm_dist <= 0.4:
                    score = max(score, 1.0)
                elif best_arm_dist <= 0.8:
                    score = max(score, 0.7)
                elif best_arm_dist <= 1.4:
                    score = max(score, 0.4)
                # else: stays at 0.5 base
            ...
            dist_to_wall = self._get_min_distance_to_wall(x, y)
            half_depth = furniture['dimensions']['width'] / 2.0
            if dist_to_wall <= half_depth + 0.1:
                score = 1.0
            elif self._is_near_wall(x, y, threshold=0.5):
                score = 0.7
            else:
                # FIX: Strong penalty for wall-type items not near any wall
                score = 0.0
        
        elif primary_tag == 'corner_decor':
            # Should be in corner — strong penalty for items in open space
            if self._is_in_corner(x, y):
                score = 1.0
            elif self._is_in_corner(x, y, threshold=1.5):
                score = 0.5
            else:
                # FIX: Zero score if not near any corner (was 0.2 — too forgiving)
                score = 0.0
        ...
        elif primary_tag in ('wall_mounted', 'wall_hugger', 'leaning_decor',
                              'media_center', 'wall_seating'):
            # Must be near a wall — same hard gradient as wall_unit
            dist_to_wall = self._get_min_distance_to_wall(x, y)
            half_depth = furniture['dimensions']['width'] / 2.0
            if dist_to_wall <= half_depth + 0.1:
                score = 1.0
                ...
                score = 0.0   # hard penalty — not acceptable away from wall

        elif primary_tag == 'requires_surface':
            # table_lamp: reward is based on proximity to a surface furniture
            surface_types = {'side_table', 'console_table', 'nesting_tables'}
            all_furniture = self.existing_furniture + self.placed_items
            min_dist_to_surface = min(
                (np.sqrt((x - f['position']['x'])**2 + (y - f['position']['y'])**2)
                 for f in all_furniture if f['type'] in surface_types),
                default=999.0
            )
            if min_dist_to_surface <= 0.3:
                score = 1.0
            elif min_dist_to_surface <= 0.6:
                score = 0.6
            else:
                score = 0.0   # floating lamp — unacceptable

        elif primary_tag == 'zone_definer':
            # Rug: should be under/near seating area
            for zone in self.spatial_zones['seating_area']:
                cx, cy = zone['center']
                dist = np.sqrt((x - cx)**2 + (y - cy)**2)
                if dist <= zone['radius']:
            ...
            # storage_basket: must be in corner
            if self._is_in_corner(x, y):
                score = 1.0
            elif self._is_in_corner(x, y, threshold=1.5):
                score = 0.4
        ...
        elif primary_tag == 'unknown':
            # FIX: Unknown items (magazine_rack, mirror etc) default to wall preference
            # so they don't drift to the room centre
            if self._is_near_wall(x, y, threshold=0.6):
                score = 0.8
            elif self._is_near_wall(x, y, threshold=1.2):
                score = 0.5
            else:
                score = 0.1
    ...
    def _reward_spatial_constraints(
        ...
        # Check if must touch wall — handle all catalog key variants
        must_wall_flag = (spatial_const.get('must_touch_wall', False) or
                          spatial_const.get('must_be_on_wall', False) or
                          spatial_const.get('must_be_against_wall', False) or
                          spatial_const.get('must_lean_on_wall', False) or
                          spatial_const.get('against_wall', False))
        if must_wall_flag:
            half_depth = furniture['dimensions']['width'] / 2.0
        ...
        # Check corner requirements — handle all three catalog key variants
        corner_flag = (spatial_const.get('strongly_prefers_corner', False) or
                       spatial_const.get('corner_preferred', False) or
                       spatial_const.get('prefers_corner', False))
        if corner_flag:
            if self._is_in_corner(x, y):
                score = max(score, 1.0)
            elif self._is_in_corner(x, y, threshold=1.5):
                score = min(score, 0.4)   # FIX: was min(score, 0.4) — kept
            else:
                # FIX: hard penalty to force corner
                score = 0.0
    ...
    def _map_type_to_primary_tag(self, ftype: str) -> str:
        """Map furniture type to primary tag — mirrors actual catalog primary_tag values"""
        mapping = {
            # Seating
            'sofa': 'primary_seating',
            'armchair': 'primary_seating',
            'ottoman': 'flexible_seating',
            'pouf': 'flexible_accent',
            'bench': 'wall_seating',
            # Seating companions / accessories
            'coffee_table': 'seating_companion',
            'side_table': 'seating_accessory',
            'nesting_tables': 'seating_accessory',
            'magazine_rack': 'seating_accessory',   # beside sofa, NOT wall item
            # Lighting
            'floor_lamp': 'task_lighting',
            'table_lamp': 'requires_surface',        # needs a table beneath it
            # Wall items
            'bookshelf': 'wall_unit',
            'console_table': 'wall_hugger',
            'wall_shelf': 'wall_mounted',
            'mirror': 'wall_mounted',
            'media_cabinet': 'media_center',
            'decorative_ladder': 'leaning_decor',
            # Corner items
            'plant_stand': 'corner_decor',
            'storage_basket': 'corner_storage',
            'bar_cart': 'corner_storage',
            # Other
            'rug': 'zone_definer',
        }
