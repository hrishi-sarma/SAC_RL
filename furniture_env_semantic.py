...
class FurnitureRecommendationEnvSemantic(gym.Env):
    ...
    def _check_semantic_constraints(self, furniture: Dict, catalog_item: Dict) -> bool:
        ...
        functional_props = catalog_item.get('functional_properties', {})
        
        # 1. WALL-MOUNTED ITEMS (shelves, mirrors) must be near walls
        if functional_props.get('wall_mounted', False):
            ...
            if item_type in ['mirror', 'wall_shelf']:
                if self._is_near_window(x, y, 0.8) or self._is_near_door(x, y, 1.0):
                    return False
        
        # 2. PLANT STANDS should be in corners or near windows
        if item_type == 'plant_stand':
            near_corner = self._is_near_corner(x, y, 1.5)  # Relaxed from 1.0 to 1.5
            near_window = self._is_near_window(x, y, 2.0)  # Relaxed from 1.5 to 2.0
            if not (near_corner or near_window):
                return False
        
        # 3. FLOOR LAMPS near seating areas
        if item_type == 'floor_lamp':
            if not self._is_near_seating(x, y, 3.5):  # Relaxed from 2.0 to 3.5
                return False
        
        # 4. TABLE LAMPS must be on tables
        if item_type == 'table_lamp':
            if not self._is_on_table(x, y):
                return False
        
        # 5. RUGS should be central, under/near seating
        if item_type == 'rug':
            if not self._is_near_seating(x, y, 4.0):  # Relaxed from 3.0 to 4.0
                return False
        
        # 6. STORAGE near walls (not in center)
        if item_type in ['bookshelf', 'console_table', 'media_cabinet']:
            if not self._is_near_wall(x, y, 1.2):  # Relaxed from 0.8 to 1.2
                return False
        
        # 7. DECORATIVE LADDERS lean on walls
        if 'ladder' in item_type:
